...
O termo surgiu em meados de 1960 e se propagou com mais força nas décadas
subsequentes, uma das razões é que, na época em que os mainframes dominavam o
cenário tecnológico e não haviam computadores pessoais, por exemplo, não existia a
praticidade de "adquirir, instalar e usar um software" - este era acompanhado de
bibliotecas e outros recursos que o tornavam quase que exclusivos ao o computador para
o qual foi desenvolvimento originalmente. Desta forma, muitas vezes uma organização
que implementava um novo sistema se via obrigada a adquirir um equipamento apenas
para executá-lo, em vez de simplesmente aproveitar o maquinário existente, deixando
toda a operação mais cara no final das contas. A virtualização conseguiu resolver este
problema: pode-se aproveitar um computador já existente para executar dois ou mais
sistemas distintos, já que cada um executa dentro de sua própria máquina virtual. Evita-
se, assim, gastos com novos equipamentos e aproveita-se os possíveis recursos ociosos
do computador.
>>>>>>> Stashed changes
...
\begin{figure}
  ...
  \caption{Comparativo do uso de processamento entre serviços virtualizados em
  relação as máquinas tradicionais.}
  Fonte: \url{http://www.infowester.com/virtualizacao.php}
\end{figure}
...
É um protocolo de comunicação para visualização remota de interfaces, ele é
retrocompatível com o RDP da Microsoft. Este protocolo funciona sobre o protocolo TCP
e por padrão na porta 3389. Através deste protocolo pode-se ter acesso a Desktops
virtualizados através da rede, permitindo que vários usuários possam compartilhar a
mesma tela ou acessar telas diferentes da mesmas máquina virtual.

\subsubsection{Teletransporte}
O VirtualBox tem uma ferramenta para a realização de transferência de máquinas virtuais,
através desta ferramente é possível que uma máquina virtual possa ser transferida de um
computador para outro através da rede.

\subsubsection{VBoxManage}
É um programa para a realização de controle de máquinas virtuais através da linha de
comando, com isto é possível construir scripts que realizam a chamada deste programa afim
de automatizar processos de construção, clonagem e quaisquer outras alterações em máquinas

\section{Proposta de Implementação}
A implementação consistirá de uma API REST HTTP para criação, escalonamento,
clonagem, compartiilhamento e destruição de máquinas virtuais, utilizando a
tecnologia do Oracle VM VirtualBox e o protocolo de comunicação VRDP. O
objetivo é permitir que múltiplos ambientes de desktop sejam gerenciados e
escalonados sem a necessidade de preocupação com o hardware usado por parte do
cliente.
